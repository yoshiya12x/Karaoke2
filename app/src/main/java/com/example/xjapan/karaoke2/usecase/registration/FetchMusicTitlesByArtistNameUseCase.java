 ...
 */
public class FetchMusicTitlesByArtistNameUseCase implements FetchMusicTitlesUseCase {
    ...
    public void apply(final String query, final Paging paging) {
        service.submit(new Runnable() {
            ...
            public void run() {
                try {
                    List<MusicTitle> musics = AppClient.sync().fetchMusicTitlesByArtistName(query, paging.getNumber(), paging.getLimit());
